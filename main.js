(function () {
  ...
  const PLAYER_SHEET = (typeof window!=='undefined'&&window.PLAYER_SHEET)?window.PLAYER_SHEET:null;
  const MIGS_SHEET = (typeof window!=='undefined'&&window.MIGS_SHEET)?window.MIGS_SHEET:null;
  ...
  class MainScene extends Phaser.Scene {
    ...
    preload(){
      const maybeLoadSheet=(sheet)=>{
        if(!sheet||!sheet.key||!sheet.url)return;
        ...
        this.load.spritesheet(sheet.key,sheet.url,{
          ...
          endFrame:sheet.endFrame===undefined?-1:sheet.endFrame,
        });
      };
      maybeLoadSheet(PLAYER_SHEET);
      maybeLoadSheet(MIGS_SHEET);
    }
