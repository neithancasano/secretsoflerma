(function () {
  ...
  // NPC visual configs — add new monster types here later!
  const NPC_VISUALS = {
  ...
  // ── NPC entity ──
  function upsertNPC(scene, id, tx, ty, name, kind) {
    let n = STATE.npcs.get(id);
    const vis = NPC_VISUALS[kind] || NPC_VISUALS.poring;

    if (!n) {
      const s = vis.scale || 1;
      // Porings are round blobs — bigger ellipse, smaller body
      ...
      const container = scene.add.container(0, 0, [base, body, eyeL, eyeR, pupL, pupR, label]);
      n = { id, tx, ty, rx: tx, ry: ty, sprite: container, label, kind };
      STATE.npcs.set(id, n);
    }
    n.tx = tx; n.ty = ty;
  }
  ...
  function setDepth(e) {
    e.sprite.setDepth((e.ty + e.tx) * 10 + 4); // NPCs slightly below players
  }
  ...
  class MainScene extends Phaser.Scene {
    ...
    onNet(msg) {
      if (msg.t === "WELCOME") {
        ...
        fetch('/secretsoflerma/map.json')
          .then(r => r.json())
          .then(data => { STATE.map.tiles = data.tiles; this.drawTileMap(); })
      ...
      if (msg.t === "SNAPSHOT") {
        ...
        for (const id of Array.from(STATE.npcs.keys())) removeNPC(id);
        ...
        // Render NPCs from snapshot
        if (Array.isArray(msg.npcs)) {
          for (const n of msg.npcs) upsertNPC(this, n.id, n.x, n.y, n.name, n.kind);
          for (const n of STATE.npcs.values()) setDepth(n);
        }
      ...
      if (msg.t === "DELTA") {
        ...
        // Update NPCs from delta
        if (Array.isArray(msg.npcUp)) {
          for (const n of msg.npcUp) upsertNPC(this, n.id, n.x, n.y, n.name, n.kind);
          for (const n of STATE.npcs.values()) setDepth(n);
        }
        return;
      }
    ...
    update(_, dtMs) {
      ...
      // Smooth NPC movement — Porings move slower so use gentler follow
